...
/// `course_lines` が `Some` なら棒を文字壁で描く（term `--cmd`）。
fn draw_scene(
    out: &mut impl Write,
    game: &Game,
...
/// `--cmd <command>` を取り出す。未指定なら `None`（通常プレイ）。値の欠落は usage 誘導つきで
/// 非ゼロ終了（silent fallback しない）。
fn parse_cmd(args: &[String]) -> Option<String> {
    let pos = args.iter().position(|a| a == "--cmd")?;
    match args.get(pos + 1) {
        Some(s) => Some(s.clone()),
        None => {
            eprintln!("error: --cmd には値が必要です（--help 参照）");
            std::process::exit(2);
        }
    }
}

/// `--cmd` のコマンドを実行し、文字壁コースの行（空でない）を返す。
/// 実行失敗・使える行が 0 なら usage 誘導つきで非ゼロ終了する（alternate screen 入場前に呼ぶ）。
fn course_lines_or_exit(cmd: &str) -> Vec<String> {
    let lines = course::run_command(cmd).unwrap_or_else(|e| {
        eprintln!("error: コマンド実行に失敗しました: {e}（--help 参照）");
        std::process::exit(2);
    });
    if lines.is_empty() {
        eprintln!("error: コマンド出力に使える行がありません: {cmd:?}（--help 参照）");
        std::process::exit(2);
    }
    lines
}
...
fn main() -> io::Result<()> {
    ...
    // --cmd モード: コマンドを実行して文字壁コースを作る（alternate screen 入場“前”に行い、
    // エラーは通常のターミナルへ出す）。未指定なら None（通常の rng コース）。
    let course_lines: Option<Vec<String>> = parse_cmd(&args).map(|cmd| course_lines_or_exit(&cmd));
