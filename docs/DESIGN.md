...
pub struct Config {            // チューニング値はここに集約
    ...
    pub scroll_speed: f32,     // 列/秒（speedup の初速 base でもある）
    pub scroll_speed_step: f32,// score 1 本あたりの速度上昇量（列/秒）。既定 0.0 = 上昇なし
    pub scroll_speed_max: f32, // 速度上限（列/秒）。既定 = scroll_speed（= 上昇なし）
    pub pipe_gap: u16,         // 隙間の縦幅（行）
    pub pipe_spacing: f32,     // 棒の間隔（列）
    pub vy_max: f32,           // 終端速度（下向き上限、行/秒）
}
// Config::with_speedup() で step/cap を載せる（term/web 共有の単一ソース。§4/§5/§7）。
// 実効速度 current_scroll_speed = (scroll_speed + step * score).min(cap)（score 依存・純粋）。
pub enum Phase { Ready, Playing, GameOver }
pub struct Pipe { pub x: f32, pub gap_top: u16, pub passed: bool,
    pub course_idx: usize }    // term --cmd で「どの行の文字で描くか」を引く添字（rng モードは 0）
pub struct Game {
    cfg: Config, rng: Rng, phase: Phase,
    bird_y: f32, bird_vy: f32,
    pipes: Vec<Pipe>,
    dist_to_next: f32,         // 次の棒生成までの距離
    score: u32, best: u32,     // 読み取りは score()/best() ゲッター経由（加点は tick 内のみ）
    course: Vec<u16>,          // term --cmd の gap_top 列（空なら rng 抽選）
    course_pos: usize,         // course の次に読む位置（restart で 0 に戻り先頭から再生）
}
```

### API（core が公開する最小操作）
- `Game::new(cfg, seed) -> Game`
- `Game::with_course(cfg, seed, course: Vec<u16>) -> Game` — `Game::new` に棒の `gap_top` 列（course）を与える版（term の `--cmd`、§4）。`course` 非空なら `spawn_pipe` は rng でなくこの列を順に消費（尽きたら先頭へループ）し各値を `[1, rows-1-pipe_gap]` にクランプ、空なら `Game::new` と完全同一（rng 抽選）
...
> 衝突は移動後セルの離散判定で十分。**横**: 固定ステップなので棒の 1 フレーム移動は `scroll_speed × DT`。不変条件 **`scroll_speed × DT < 1.0`**（DT=1/60 なら `scroll_speed < 60`）が棒幅 1 列を割るので、棒は鳥列に必ず 1 フレーム重なる。**縦**: `vy_max` キャップで 1 フレームの落下が `vy_max × DT < 1 行` に収まる。両軸とも「1 フレーム 1 セル以内」なのでスイープ判定は不要。`new()` で `assert!(scroll_speed * DT < 1.0 && vy_max * DT < 1.0)` し、§7 の体感チューニングで不変条件が破れないよう担保する。加えて `new()` は `assert!(pipe_gap + 2 <= rows)` で `gap_top` の抽選範囲 `[1, rows-1-pipe_gap]` が空（＝`rows-1-pipe_gap` の u16 アンダーフロー）にならないことも保証する。
...
| scroll_speed | 12.0 列/s | 横スクロール速度（speedup の初速 base） |
| scroll_speed_step | 0.0 列/s | score 1 本あたりの速度上昇量。既定 0（上昇なし）。`with_speedup` で 1.5 |
| scroll_speed_max | 12.0 列/s | 速度上限。既定 = scroll_speed（上昇なし）。`with_speedup` で 30（< 60 で不変条件内） |
| pipe_gap | 6 行 | 隙間の縦幅 |
| pipe_spacing | 22.0 列 | 棒の間隔 |
| vy_max | 30.0 行/s | 終端速度（下向き上限。`vy_max × DT < 1` 行/フレーム） |

棒は固定で **1 列幅**（`pipe_width` は Config 化しない）。境界は HUD=行0・地面=行 `rows-1`（鳥が乗れる範囲 `1..=rows-2`）。隙間 `gap_top` は `[1, rows-1-pipe_gap]`（両端含む）から rng で選ぶ。初期化時は最初の棒を `x = cols` に置き `dist_to_next = pipe_spacing` から開始（1本目到達まで約1画面の助走、開始即死を防ぐ）。**不変条件 `scroll_speed × DT < 1.0` かつ `vy_max × DT < 1.0`**（DT=1/60 → scroll<60。離散セル判定の取りこぼし防止）を `new()` で assert する。あわせて `gap_top` の抽選範囲 `[1, rows-1-pipe_gap]` が空にならないよう **`pipe_gap + 2 <= rows`** も `new()` で assert する（破れると `rows-1-pipe_gap` が u16 アンダーフローして棒が画面外に置かれる）。

数値は「人が数本くぐれる」体感で最終調整する（成功条件）。最高スコアはセッション内メモリ保持のみ（ファイル永続化は v1 では入れない）。

**難易度漸増（速度up）は opt-in モードで実装済み**（term `--speedup` / web `?speedup=1`。§4/§5）。Config に寄せてあった前提（`scroll_speed`）を活かし、score 依存の実効速度を供給する 2 フィールド（`scroll_speed_step` / `scroll_speed_max`）と純粋ヘルパ `current_scroll_speed` を後付けした。既定 Config は step=0・cap=`scroll_speed` で実効速度が一定＝**従来挙動・既存ゴールデンは不変**（後方互換）。上昇の駆動は時間でなく **score ベース**（整数のゲーム状態なので決定論を完全に保つ）。

> **将来メモ（衝突判定の切り替え）**: 衝突は v1 では離散の等値判定（`pipe_col == bird_c`）で、不変条件 `scroll_speed × DT < 1.0`（DT=1/60 → `scroll_speed < 60`）の範囲でのみ正しい。`--speedup` モードは cap を **60 列/秒未満（現状 30）に抑えて等値判定のまま据え置く**（`new()` の assert を `scroll_speed_max × DT < 1.0` にも拡張し、cap ≥ 60 の Config を構築時に弾く）。将来 `scroll_speed` をこの上限に近づける／超える高速モードを足す場合は、**等値判定を区間判定（CCD: 移動前 x と移動後 x で鳥列 `[bird_col-0.5, bird_col+0.5)` を跨いだかで判定）に切り替える**こと。切り替えれば速度上限は消える。縦も同様に `vy_max × DT < 1.0` を維持する（超えるなら縦も掃く）。現状は `new()` の assert がこの閾値超えを検出するので、越えた時点で着手すればよい。
