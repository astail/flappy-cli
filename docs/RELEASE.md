...
| 対象 | 役割 |
|---|---|
| `crates/core/Cargo.toml` | 画面表示の version（`flappy_core::VERSION` = `CARGO_PKG_VERSION` 由来） |
| `crates/term/Cargo.toml` | `release.yml` が **タグと照合する単一ソース** |
| `crates/web/Cargo.toml` | web 版（独立 workspace） |
| `crates/term/src/golden/ready.txt` | golden test が画面下部の `vX.Y.Z` 表示を検証。**忘れると CI が落ちる** |
...
- [ ] `Cargo.lock` × 2 を同期
- [ ] `crates/term/src/golden/ready.txt` の version 表示を更新
